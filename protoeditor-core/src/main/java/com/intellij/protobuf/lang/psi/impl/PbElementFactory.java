...
public class PbElementFactory {
  ...
  private PsiElement parseLight(IElementType type, CharSequence text) {
    ...
    LanguageVersion defaultVersion = LanguageVersionUtil.findDefaultVersion(PbLanguage.INSTANCE);
    PsiBuilder builder =
        PsiBuilderFactory.getInstance()
            .createBuilder(parserDefinition, parserDefinition.createLexer(defaultVersion), defaultVersion, text);
    PbParser parser = (PbParser) parserDefinition.createParser(defaultVersion);
